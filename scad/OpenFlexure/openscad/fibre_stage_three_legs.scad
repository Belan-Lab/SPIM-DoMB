...
module casing_outline(cubic=true){
    ...
    if(cubic){
        translate([-stage[0]/2-casing_size_s, z_anchor_bottom_y-wall_t, 0])
                cube(casing_size);
    }else{
        minkowski(){
            hull() mechanism_void();
    ...
    difference(){
        ...
        difference(){
            ...
            rotate(-135) translate([0,so+5,platform_z]) 
                ...
                repeat([0,10,0],10) trylinder_selftap(3,h=20,center=true);
...
module moving_platform(){
    ...
    difference(){
        ...
        for (h_pos=[[5,5],[-5,-5],[-15,5],[-15,-5]]) translate([h_pos[0],h_pos[1],stage_top + 3]){
            cylinder(d=3.3*1.15,h=999, center=true);
            cylinder(d=7*1.15,h=999);
        }
...
module fixing_lug(isbase=false){
  //makes a fixing lug to connect the base to the main body
  difference()
  {
      hull(){
          translate([0, 5,0 ])cylinder(d=8,h=3);
          translate([0, -5,0 ])cylinder(d=8,h=7);
      }
      translate([0, 5,3])cylinder(d=6,h=999);
      if(isbase){
          translate([0, 5,0])trylinder_selftap(3,h=999,center=true);
      }
      else{
          translate([0, 5,0])cylinder(d=3.2*1.2,h=999,center=true);
      }
  }
}

module casing(mechanism_void=true,isbase=false){
    ...
    difference(){
        union(){
            ...
            translate([casing_size[0]/2-4, -casing_size[1]/2,0 ])rotate([0,0,180])fixing_lug(isbase=isbase);
            translate([-(casing_size[0]/2-4), -casing_size[1]/2,0 ])rotate([0,0,180])fixing_lug(isbase=isbase);
            translate([-casing_size[0]/2, casing_size[1]/2-9,0 ])rotate([0,0,90])fixing_lug(isbase=isbase);
        }
        // limit the wall in Z
        translate([-(casing_size[0]+4)/2,-(casing_size[1]+4)/2,shelf_z2 + stage[2] - z_travel]) cube([casing_size[0]+4,casing_size[1]+4,999]);
