...
beam_height = 100; // 75 <<<<<< height of beam above the table
stage_to_beam_height = 0; // 12.5 <<<<<<
...
//fixed_platform = false; // this is overwritten later on with the dimensions anyway.
...
flex_a = 0.2; //angle through which flexures are bent, radians, 0.1 <<<<<
...
platform_z = shelf_z2 + stage[2] + platform_z_add; // 7 <<<<<<< The platform is a fixed height above the moving stage
fixed_platform_standoff = 10;
fixed_platform = [50,40,4];
platform_gap = xy_travel + 1;

//term for calculating casing size
casing_size_s = xy_bottom_travel + zflex[1] + zflex[0] + 0.5 + wall_t;
//casing size
casing_size = stage + [2*casing_size_s,casing_size_s + (-z_anchor_bottom_y-stage[1]/2) + wall_t,shelf_z2];
...
//foot_height is required for the actuator assembly tools
foot_height = 20;
