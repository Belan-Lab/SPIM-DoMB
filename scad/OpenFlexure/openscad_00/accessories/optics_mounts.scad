...
module openflexure_microscope_module(rms_mount_l=15, infinite_conjugates=false){
    ...
    top_z = 75-45-rms_mount_l; // sample_z is 75mm, minus 45mm parfocal distance.
    echo("top z is ", top_z);
    outer_r = rms_mount_hole_spacing/sqrt(2)+3;
    inner_r = 17.5; // this should be small enough to fit inside the RMS thread but
    ...
    difference(){
        ...
        translate([0,0,top_z]) difference(){
            cylinder(r=99,h=99,$fn=3);
            cylinder(d=inner_r,h=99,center=true);
        }
